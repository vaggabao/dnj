...
/**
 * handles methods for managing bills
 * @author Sire
 * @link null
 * @link null
 * @license null
...
class Billing
{
    ...
    {   
        if ($this->databaseConnection()) {
            ...
            if (isset($result_row->id)) {
                ...
                $query_transactions = $this->db_connection->prepare("SELECT * FROM tbl_bills WHERE tenant_id = :tenant_id AND payment_status NOT LIKE 'cancelled' ORDER BY id ASC");
    ...
    /**
     * Retrieve details of a billing period in the database
     */
    public function getBillingDetails($billing_id) {   
        if ($this->databaseConnection()) {
            $query_billing = $this->db_connection->prepare("SELECT * FROM tbl_billing_description WHERE billing_id = :billing_id ORDER BY id ASC");
            $query_billing->bindValue(':billing_id', $billing_id, PDO::PARAM_INT);
            $query_billing->execute();

            $result_row = $query_billing->fetchAll();
			return $result_row;
        }
    }
}
